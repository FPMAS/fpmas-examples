...
int main(int argc, char** argv) {
	...
	{
		// GridLoadBalancing algorithm
		fpmas::model::GridLoadBalancing grid_load_balancing(
				5, 5, fpmas::communication::WORLD
				);


		// Grid Model initialized with zoltan, so that model.loadBalancingJob()
		// is bound to Zoltan
		fpmas::model::GridModel<fpmas::synchro::GhostMode> model(grid_load_balancing);
		...
		fpmas::model::UniformGridAgentMapping agent_mapping(5, 2, 1);
